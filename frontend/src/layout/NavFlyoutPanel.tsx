...
const plusBtnClass =
  "relative z-[210] flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 shadow-sm transition-colors hover:border-blue-300 hover:bg-blue-50 hover:text-blue-800";
...
function FlyoutRow({
  ...
  const linkPartClass = `flex min-h-[40px] min-w-0 flex-1 items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors [&_svg]:shrink-0 ${
    rowActive ? "bg-blue-100 text-blue-900" : "text-slate-700 hover:bg-blue-50"
  }`;

  const iconWrap = (
    <span className="text-slate-500 [&_svg]:h-[18px] [&_svg]:w-[18px]">
  ...
  if (plusTarget) {
    return (
      <div
        className={`flex min-h-[40px] items-center gap-1 rounded-md py-0.5 pr-1 pl-0.5 transition-colors ${
...
export default function NavFlyoutPanel({
  ...
  return (
    ...
    >
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain p-4 [-webkit-overflow-scrolling:touch] [scrollbar-width:thin] [scrollbar-gutter:stable] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300/90">
        {category.flyoutSections.map((section, si) => (
          <div key={si}>
            {si > 0 ? <div className="my-2 border-t border-slate-200" role="separator" /> : null}
            {section.title ? (
              <div className="mb-1.5 px-1 pt-0.5 text-[11px] font-bold uppercase tracking-wide text-slate-400">
                {section.title}
              </div>
            ) : null}
