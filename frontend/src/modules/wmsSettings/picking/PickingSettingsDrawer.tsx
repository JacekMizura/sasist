...
/** Right-side edit drawer for picking mode / status configuration (presentation only). */
export function PickingSettingsDrawer({ open, title, subtitle, onClose, children }: Props) {
  if (!open) return null;
  return createPortal(
    <div className="fixed inset-0 z-[120]" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <aside className="absolute inset-y-0 right-0 flex w-full max-w-xl flex-col border-l border-slate-200 bg-white shadow-2xl">
        ...
        <footer className="shrink-0 border-t border-slate-100 bg-slate-50 px-5 py-3 text-xs text-slate-500">
