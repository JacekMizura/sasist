...
function ShellInner({ children }: { children: ReactNode }) {
  const { activeSectionId, selectSection } = useWmsSettingsSectionRegistry();
  return (
    <div className="grid grid-cols-1 gap-5 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-start">
      <aside className="relative min-h-0" aria-label="Sekcje ustawień zbierania">
        <div className="lg:sticky lg:top-4">
          <PickingSettingsSectionNav activeSectionId={activeSectionId} onSelect={selectSection} />
        </div>
      </aside>
 ...
 * Left nav switches the active section (same registry as WmsSettingsLayout).
 */
