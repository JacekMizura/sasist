...
export default function WmsInventoryProductPreview({ scan, pulse, invalid }: Props) {
  ...
  return (
    <div className={`py-0.5 transition-all ${ring}`}>
      <div className="flex gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-bold leading-snug text-[#1a2b3c]">{scan.product_name ?? "—"}</p>
      ...
      <p key={qty} className="mt-0.5 text-right text-4xl font-black tabular-nums leading-none text-[#1e4d8c]">
        {qty}
      </p>
    </div>
  );
