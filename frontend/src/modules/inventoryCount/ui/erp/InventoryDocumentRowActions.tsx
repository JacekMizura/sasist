...
export function InventoryDocumentRowActions({
  ...
  const hasOverflow = Boolean(onDuplicate || onExport);
        ...
        <OperationalActionColumn
          ...
          slots={[
            ...
            <OperationalActionLink
              ...
              title={editable ? "Edytuj" : "Podgląd"}
              aria-label={editable ? `Edytuj ${doc.number}` : `Podgląd ${doc.number}`}
            >
            ...
            hasOverflow ? (
              <button
                ...
                aria-expanded={open}
                aria-haspopup="menu"
                aria-label={`Więcej akcji ${doc.number}`}
                title="Więcej"
                onClick={() => setOpen((v) => !v)}
