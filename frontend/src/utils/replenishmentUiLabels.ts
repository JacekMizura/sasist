...
export const REPLENISHMENT_CLASSIFICATION_LABEL_PL: Record<string, string> = {
  ...
  NO_SOURCE_STOCK: "Brak stanu źródłowego",
...
export function opsAlertLevelLabel(code: string | null | undefined): string {
  const k = String(code || "")
    .trim()
    .toLowerCase();
  if (!k) return "Powiadomienie";
  return OPS_ALERT_LEVEL_LABEL_PL[k] ?? "Powiadomienie";
}
...
export function opsSeverityLabel(code: string | null | undefined): string {
  const k = String(code || "")
    ...
    .toLowerCase();
  if (!k) return "Stan";
  return OPS_SEVERITY_LABEL_PL[k] ?? "Stan";
}
...
export function opsResolutionStatusLabel(code: string | null | undefined): string {
  const k = String(code || "")
    ...
    .toLowerCase();
  if (!k) return "Stan";
  return OPS_RESOLUTION_STATUS_LABEL_PL[k] ?? "Stan";
}
