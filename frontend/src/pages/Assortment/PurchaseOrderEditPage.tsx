...
export default function PurchaseOrderEditPage() {
  ...
  const { orderId: idParam, tab: tabParam } = useParams<{ orderId?: string; tab?: string }>();
  ...
  const orderId =
    idParam != null && /^\d+$/.test(idParam) ? Number(idParam) : null;
  const tab = parsePurchaseOrderEditTab(tabParam);
  const listHref = tenantId >= 1 ? `/goods-orders?tenant_id=${tenantId}` : "/goods-orders";
  ...
  useEffect(() => {
    if (orderId == null || tab !== "products" || supplierId < 1) {
      setSupplierCatalog([]);
      setTopProducts([]);
  ...
  }, [orderId, tab, tenantId, supplierId, manufacturerFilterId, catalogScope]);
  ...
  const handleDeleteDraft = async () => {
    if (order?.status !== "draft" || orderId == null) return;
  ...
  if (orderId == null) {
    return (
      <AssortmentEntityPageShell
        breadcrumbs={[
          { label: "Zakupy i planowanie", to: "/purchasing" },
          { label: "Zamówienia towaru", to: listHref },
          { label: "Nie znaleziono" },
        ]}
        title="Zamówienie"
        backTo={listHref}
        backLabel="Lista zamówień"
      >
        <p className="text-sm text-slate-600">Nieprawidłowy identyfikator zamówienia.</p>
      </AssortmentEntityPageShell>
    );
  }
  ...
  const pageTitle = order?.name?.trim() ? order.name.trim() : `#${orderId}`;
  const breadcrumbTail = order?.name?.trim() || `#${orderId}`;
  ...
  return (
    <AssortmentEntityPageShell
      ...
      title={pageTitle}
      subtitle={order ? `${order.supplier_name} · #${orderId}` : `ID: ${orderId}`}
      backTo={listHref}
      backLabel="Lista zamówień"
      tabs={purchaseOrderDetailTabs(orderId, tenantId)}
      footer={pageFooter}
    >
      {loadErr ? (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">{loadErr}</div>
      ) : null}
      ...
      {order ? (
        <div className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex shrink-0 flex-col gap-3 lg:max-w-sm lg:items-end">
              {supplierInsight ? (
                <div className="w-full rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 text-sm lg:min-w-[220px]">
              ...
              <button
                ...
                className="w-full shrink-0 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-800 hover:bg-slate-50 disabled:opacity-50 lg:w-auto"
              >
