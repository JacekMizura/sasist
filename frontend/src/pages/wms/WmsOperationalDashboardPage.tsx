...
export default function WmsOperationalDashboardPage() {
  ...
  return (
    <div className="min-h-full bg-[#0f172a] px-4 py-4 text-white sm:px-6">
          ...
          <>
            <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-2xl border border-slate-700 bg-slate-800/80 p-3">
                ...
                <p className="mt-1.5 text-xl font-black">{stats.total}</p>
                <p className="text-xs text-slate-400">Aktywne operacje</p>
              </div>
              <div className="rounded-2xl border border-violet-700/50 bg-violet-950/50 p-3">
                <Truck className="text-violet-300" size={20} />
                <p className="mt-1.5 text-xl font-black">{stats.relocActive}</p>
                <p className="text-xs text-violet-200">Relocation w toku</p>
              </div>
              <div className="rounded-2xl border border-amber-700/50 bg-amber-950/40 p-3">
                <Timer className="text-amber-300" size={20} />
                <p className="mt-1.5 text-xl font-black">{fmtQty(stats.waitingQty)}</p>
                <p className="text-xs text-amber-100">Szt. na dostawę</p>
              </div>
              <div className="rounded-2xl border border-emerald-700/50 bg-emerald-950/40 p-3">
                <Activity className="text-emerald-300" size={20} />
                <p className="mt-1.5 text-xl font-black">{stats.inProgress}</p>
                <p className="text-xs text-emerald-100">Sesje in_progress</p>
              </div>
            </div>

            <section className="mt-4 rounded-2xl border border-slate-700 bg-slate-800/60 p-3">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2">
                {OPERATIONAL_QUEUES.map((q) => {
                  ...
                  return (
                    <div
                      key={q.id}
                      className={`flex items-center justify-between rounded-xl px-3 py-2 ${intensity}`}
                    >
                      <div>
