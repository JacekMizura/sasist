  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs font-medium text-slate-600">Zakres wykresu</label>
          <select
            className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm shadow-sm"
            value={rangeDays}
      ...
      {loading && !data ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm ring-1 ring-slate-200/80">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Analizowane produkty</p>
              <p className="mt-2 text-3xl font-semibold tabular-nums text-slate-900">{s.products_analyzed}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm ring-1 ring-slate-200/80">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Średni zapas (dni)</p>
              <p className="mt-2 text-3xl font-semibold tabular-nums text-slate-900">
                {s.avg_stock_cover_days != null ? s.avg_stock_cover_days : "—"}
              </p>
              <p className="mt-1 text-xs text-slate-500">Średnia z pokryć przy sprzedaży &gt; 0</p>
            </div>
            <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-5 shadow-sm ring-1 ring-amber-200/80">
              <p className="text-xs font-semibold uppercase tracking-wide text-amber-900">Produkty ryzyka</p>
              <p className="mt-2 text-3xl font-semibold tabular-nums text-amber-950">{s.risk_products_count}</p>
              <p className="mt-1 text-xs text-amber-900/80">Pokrycie &lt; 7 dni</p>
            </div>
            <div className="rounded-xl border border-violet-200 bg-violet-50/60 p-5 shadow-sm ring-1 ring-violet-200/80">
              <p className="text-xs font-semibold uppercase tracking-wide text-violet-900">Martwy stock</p>
              <p className="mt-2 text-3xl font-semibold tabular-nums text-violet-950">{s.dead_stock_count}</p>
              <p className="mt-1 text-xs text-violet-900/80">Stan &gt; 0, brak sprzedaży 60+ dni</p>
            </div>
          </div>

          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-sm font-semibold text-slate-900">Wolumen sprzedaży (szt. / mies.)</p>
            <p className="text-xs text-slate-500">
              Ekstrapolacja: (suma szt. w wybranym oknie {rangeDays} dni ÷ {rangeDays}) × 30 ={" "}
              <span className="font-medium tabular-nums text-slate-800">{s.total_monthly_sales.toLocaleString("pl-PL")}</span>
            </p>
            <p className="mt-1 text-xs text-slate-500">
              Wartość magazynu (szac. koszt):{" "}
              <span className="font-medium tabular-nums">
                {s.total_stock_value.toLocaleString("pl-PL", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </span>
            </p>
          </div>

          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900">Wykres trendu sprzedaży</h2>
            <p className="text-xs text-slate-500">Suma dzienna: ilość i przychód z linii zamówień (bez statusu REPLACED).</p>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900">Top rotacja (30 dni)</h2>
            ...
            <p className="mt-2 text-xs text-slate-500">Kliknij słupek, aby wczytać produkt w inspektorze.</p>
          </section>

          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900">Produkty ryzyka</h2>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900">Martwy stock</h2>
            <div className="mt-3 overflow-x-auto">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-slate-900">Inspektor produktu</h2>
