...
export default function HotProductsPage() {
  const [items, setItems] = useState<{ product_id: number; product_name?: string; total_quantity: number }[]>([]);
  ...
  const runSearch = useCallback(() => {
    setLoading(true);
    setError(null);
    getHotProducts(DEFAULT_TENANT_ID, {
      name: name || undefined,
      ean: ean || undefined,
      sku: sku || undefined,
      limit,
  ...
  useEffect(() => {
    setLoading(true);
    setError(null);
    getHotProducts(DEFAULT_TENANT_ID, { limit: DEFAULT_LIMIT })
      .then(setItems)
      .catch((e) => setError(e?.message ?? "Błąd ładowania"))
      .finally(() => setLoading(false));
  }, []);
      ...
      <div className="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {items.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-6 text-center text-slate-500">Brak zamówień.</td></tr>
            ) : (
              items.map((row, idx) => (
