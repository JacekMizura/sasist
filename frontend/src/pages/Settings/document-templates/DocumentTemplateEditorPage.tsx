...
function notifyBreadcrumbName(templateId: number, name: string) {
  window.dispatchEvent(new CustomEvent("dte-template-name-changed", { detail: { id: templateId, name } }));
}
...
export function DocumentTemplateEditorPage() {
  ...
  const load = useCallback(async () => {
    ...
    notifyBreadcrumbName(Number(templateId), data.detail.name);
  ...
  const runPublicationValidation = useCallback(async (): Promise<ValidationReport | null> => {
    ...
    const report = translateValidationReport(await validateDocumentVersion(versionId, kindCode));
  ...
  const handleNameSave = useCallback(
    async (name: string) => {
      if (!templateId) return;
      try {
        const detail = await updateDocumentTemplate(DEFAULT_TENANT_ID, Number(templateId), { name });
        setDisplayName(detail.name);
        setCtx((prev) => (prev ? { ...prev, detail: { ...prev.detail, name: detail.name } } : prev));
        notifyBreadcrumbName(Number(templateId), detail.name);
      } catch (err) {
        toast.error(extractApiErrorMessage(err, "Nie udało się zapisać nazwy szablonu."));
        throw err;
      }
    },
    [templateId],
  ...
  async function handlePublishClick() {
    try {
      const report = await runPublicationValidation();
      if (!report) {
