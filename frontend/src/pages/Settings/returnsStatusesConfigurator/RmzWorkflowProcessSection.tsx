  ...
  return (
    ...
    >
      <div className="space-y-4">
        <FormField label="Nazwa etapu">
          <Input density={FORM_FIELD_DENSITY} value={name} onChange={(e) => setName(e.target.value)} autoFocus />
        </FormField>
        <FormField label="Kolor etykiety">
          <Select density={FORM_FIELD_DENSITY} value={color} onChange={(e) => setColor(e.target.value)}>
        ...
        <IntegrationsApiPanel>
          <FormField
            label="Typ workflow (logika systemu)"
            helperText={RMZ_TYPE_OPTIONS.find((o) => o.value === type)?.hint}
          >
            <Select density={FORM_FIELD_DENSITY} value={type} onChange={(e) => setType(e.target.value as ReturnStatusType)}>
