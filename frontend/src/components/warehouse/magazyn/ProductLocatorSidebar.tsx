...
export function ProductLocatorSidebar({
  ...
  const locations = (product.assignedLocations ?? [])
    .map((a) => ({
      ...
      <div className="p-3 flex flex-col gap-3">
        <div className="flex items-start gap-3 rounded-xl border border-slate-600 bg-slate-700/80 p-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-slate-300 mt-1">Sztuki łącznie: <span className="font-mono font-semibold text-slate-100">{totalQuantity}</span></div>
            <div className="text-xs text-slate-400 mt-0.5">Podst. <span className="font-mono text-slate-300">{primaryQuantity}</span> · Rez. <span className="font-mono text-amber-300">{reserveQuantity}</span></div>
          </div>
        </div>
        <div>
