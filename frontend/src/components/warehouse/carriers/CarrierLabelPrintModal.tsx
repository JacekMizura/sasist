...
export type CarrierForLabel = { id: number; code?: string | null; barcode?: string | null };

type Props = {
  open: boolean;
  carrier: CarrierForLabel | null;
  tenantId: number;
  onClose: () => void;
};

export function CarrierLabelPrintModal({ open, carrier, tenantId, onClose }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    api
      .get<{ id: number; name: string }[]>("/labels/templates/by-type/carrier", {
  ...
  useEffect(() => {
    ...
    api
      .get<{ svg: string }>(`/label-templates/${templateId}/preview`, {
        params: { tenant_id: tenantId, preview_type: "carrier" },
      })
  ...
  const handleGenerate = async () => {
    if (carrier == null || templateId == null) return;
    setGenerating(true);
    try {
      const res = await api.post(
        "/labels/carrier",
        { carrier_id: carrier.id, template_id: templateId, quantity },
      ...
      console.error(err);
      alert("Nie udało się wygenerować PDF. Sprawdź, czy istnieje szablon typu „Nośnik”.");
    } finally {
      setGenerating(false);
    }
  };

  const title = (carrier?.code || carrier?.barcode || "").trim() || (carrier ? `#${carrier.id}` : "Nośnik");
      ...
      <div className="fixed inset-0 z-[280] flex items-center justify-center bg-black/40" onClick={onClose}>
        ...
        >
          <h3 className="text-lg font-bold text-slate-800 px-6 py-4 border-b border-slate-100">
            Drukuj etykietę — {title}
          </h3>
          <div className="p-6 space-y-4">
            {templates.length === 0 ? (
              <p className="text-sm text-amber-800 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
                Brak szablonu typu „Nośnik”. Utwórz szablon w Systemie etykiet (template_type = carrier).
              </p>
            ) : null}
            <div>
          ...
          <div className="px-6 py-4 border-t border-slate-100 flex gap-2 justify-end">
            ...
            <PrimaryButton type="button" disabled={generating || templateId == null} onClick={() => void handleGenerate()}>
