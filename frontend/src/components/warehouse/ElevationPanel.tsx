        ...
        {Array.from({ length: rack.levels }, (_, lev) => lev)
          ...
          <div key={lev} className="border border-[#E2E8F0] rounded-lg p-2 bg-slate-50">
            ...
            <div className="flex flex-wrap gap-1">
              {(binsByLevel.get(lev) ?? []).sort((a, b) => a.segment_index - b.segment_index).map((b) => {
                ...
                return (
                  <button
                    ...
                    title={`${b.label}: ${pct.toFixed(0)}% (${formatVolume(binUsedVolumeDm3(b))} / ${formatVolume(vol)} dm³)${needReplenishment ? " · Wymaga uzupełnienia" : ""}`}
                  >
