...
function HitRow({
  ...
  const meta: string[] = [];
  if (settings.show_sku && hit.sku) meta.push(hit.sku);
  if (settings.show_ean && hit.ean) meta.push(`EAN ${hit.ean}`);
  if (settings.show_catalog_number && hit.catalog_number) meta.push(`kat. ${hit.catalog_number}`);
  ...
  return (
    ...
    >
      {settings.show_product_images && hit.image_url ? (
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs text-slate-600">
          {settings.show_stock ? `dostępne: ${hit.available_qty}` : null}
          {settings.show_stock && hit.preferred_location_code ? " · " : ""}
...
export function ProductSearchDropdown({ hits, activeIndex, settings, onPick }: Props) {
  if (!hits.length) {
