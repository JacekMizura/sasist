...
function VisibilityCheckbox({
  ...
  return (
    <label className="flex cursor-pointer items-center gap-2.5 rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 transition hover:border-slate-300">
      <input
        type="checkbox"
  ...
  return (
    <div className="space-y-4">
      <div className={`${oaEditorHeaderCardClass} space-y-5`}>
        ...
        {buttonEnabled ? (
          <div className="grid gap-8 border-t border-slate-100 pt-5 lg:grid-cols-2 lg:items-start">
            <div className="max-w-md space-y-4">
              ...
              <div>
                ...
                <button
                  ...
                  className={`${oaInp} mt-1 flex w-full items-center justify-between text-left`}
            ...
            <div className="flex min-h-[12rem] flex-col rounded-xl border border-dashed border-slate-200 bg-slate-50/60 p-5">
      ...
      {buttonEnabled ? (
        <>
          <div className={`${oaEditorHeaderCardClass} space-y-3`}>
            <div>
          ...
          <div className={`${oaEditorHeaderCardClass} space-y-2`}>
            <p className="text-sm font-semibold text-slate-900">Skrót klawiaturowy</p>
            <label className={oaLbl}>
              Skrót
              <input
                type="text"
                className={`${oaInp} mt-1 max-w-sm font-mono text-sm`}
                value={manualTrigger.shortcut}
                placeholder="Ctrl+Shift+P"
                onChange={(e) => patch({ shortcut: e.target.value })}
              />
            </label>
            <p className="text-xs text-slate-500">Wyświetlany obok przycisku w podglądzie i w UI operatora.</p>
          </div>
          ...
          <div className={`${oaEditorHeaderCardClass}`}>
            <label className="flex cursor-pointer items-start gap-3 text-sm text-slate-800">
              ...
              />
              <span>
                <span className="font-semibold text-slate-900">Sprawdzaj warunki przy ręcznym uruchamianiu</span>
                <span className="mt-1 block text-slate-500">
                  Gdy włączone — przed wykonaniem weryfikowane są warunki reguły. Przy niespełnieniu akcja jest
                  blokowana. Gdy wyłączone — wykonuje się niezależnie od warunków.
                </span>
              </span>
            </label>
          </div>
