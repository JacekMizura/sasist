...
export type MultiModulePickerItem = {
  id: string;
  label: string;
  group: string;
  icon?: LucideIcon;
...
export type MultiModulePickerProps<TModuleId extends string> = {
  open: boolean;
  disabledIds?: Set<TModuleId>;
  onClose: () => void;
  onPick: (id: TModuleId) => void;
  listPickerGroups: () => { group: string; modules: MultiModulePickerItem[] }[];
  defaultIcon?: LucideIcon;
};

export function MultiModulePicker<TModuleId extends string>({
  open,
  disabledIds,
  onClose,
  onPick,
  listPickerGroups,
  defaultIcon = Scale,
}: MultiModulePickerProps<TModuleId>) {
  const [q, setQ] = useState("");
  const inputRef = useRef<HTMLInputElement>(null);
  const groups = useMemo(() => listPickerGroups(), [listPickerGroups]);
                ...
                <div key={g.group} className="mb-3 last:mb-0">
                  ...
                  <div className="mt-0.5 space-y-0.5">
                    {g.modules.map((m) => {
                      ...
                      const used = disabledIds?.has(m.id as TModuleId);
                      return (
                        <button
                          key={m.id}
                          type="button"
                        ...
                        >
                          ...
                          {used ? (
                            <span className="text-[10px] font-semibold uppercase text-slate-400">Dodano</span>
                          ) : null}
                        </button>
                      );
                    })}
