...
import { safeUpper } from "../../utils/safeStrings";
    ...
    <div className="rounded-lg border border-slate-200 bg-white">
      ...
      <ul className="max-h-64 divide-y divide-slate-50 overflow-auto">
        {rows.length === 0 ? (
          ...
          rows.map((a) => {
            const sev = safeUpper(a.severity) || "INFO";
            return (
            <li key={a.id} className="flex items-start justify-between gap-2 px-3 py-2">
              <div className="min-w-0">
                <span
                  className={`mr-2 inline-block rounded px-1 py-0.5 text-[10px] font-semibold ${SEV_CLASS[sev] ?? SEV_CLASS.INFO}`}
                >
                  {sev}
                </span>
