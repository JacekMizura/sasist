...
def test_template_rejects_two_enabled_default_passages(db):
    from fastapi import HTTPException

    from backend.services.warehouse_layout.single_passage import SINGLE_ENABLED_PASSAGE_ERROR
        ...
        svc.create(
            1,
            {
                ...
                "default_passages": [
                    ...
                    {"offset_along_cm": 200, "width_cm": 80, "clearance_height_cm": 220, "enabled": True},
                ],
            },
        )
...
def test_layout_sync_persists_passage_source_local_by_default(db):
    ...
    # One enabled + one disabled (source persistence still covers both rows).
...
def test_layout_sync_rejects_two_enabled_passages(db):
    from fastapi import HTTPException
    ...
    with pytest.raises(HTTPException) as ei:
        svc._sync_rack_passages(
            ...
            wh.id,
            [
                {"uuid": "p1", "offset_along_cm": 20, "width_cm": 40, "enabled": True},
                {"uuid": "p2", "offset_along_cm": 70, "width_cm": 30, "enabled": True},
            ],
        )
    assert ei.value.status_code == 400
    assert ei.value.detail == SINGLE_ENABLED_PASSAGE_ERROR
    assert db.query(WarehouseRackPassage).filter(WarehouseRackPassage.rack_id == rack.id).count() == 0
