...
def _apply_fleet(db: Session, tenant_id: int, warehouse_id: int) -> dict:
    """LEGACY PERSIST DISABLED — optimizer is compute-only (no cart/order lifecycle writes)."""
    raise ValueError(
        "Optimizer nie zapisuje lifecycle wozka. Uzyj trybu analizy (compute-only); "
        "przypisanie zamowien tylko przez startPicking po skanie wozka."
    )
