...
def suggest_smart_matching_v2(
    ...
    *,
    ...
    """
    Phase 1 Smart suggest:
    1) v2 product/min_qty breakpoint (single-SKU orders only)
    2) legacy v1 exact composition if legacy_v1_fallback_enabled
    Soft shipping heuristic removed from primary path (hard filter = Phase 2).
    """
    if not cartons:
        return []
    ...
    if line is not None:
        ...
        if resolved is not None and resolved.ambiguous:
            # No auto suggestion on conflict.
            return []
